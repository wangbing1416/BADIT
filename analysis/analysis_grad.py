...
from analysis_data_process import build_instruction_dataset_from_json, DataCollatorForSupervisedDataset
from analysis_trainer import QKVParameterGradientTrainer
from pathlib import Path
...
class DataTrainingArguments:
    ...
    dataset_dir: Optional[str] = field(
        default='./data/SuperNI', metadata={"help": "The name of the dataset to use (via the datasets library)."}
    )

    task_name: Optional[str] = field(
        default='task1590_diplomacy_text_generation', metadata={"help": "The name of the task."}
    )

    train_file: Optional[str] = field(default=None, metadata={"help": "The input training data file (a text file)."})
    validation_file: Optional[str] = field(
        default='./data/SuperNI',
...
class MyTrainingArguments(Seq2SeqTrainingArguments):
    ...
    lora_config_file: Optional[str] = field(default='config/lora.config')
    ...
    per_device_train_batch_size: int = field(default=1, metadata={"help": "Batch size per GPU/CPU for training."})
...
def main():
    ...
    # verify existence
    root_path = os.path.join(training_args.output_dir,
                             f'avg_grads_{data_args.task_name}_{model_args.model_name_or_path.split("/")[-1]}')
    os.makedirs(root_path, exist_ok=True)
    qkv_gradient_save_path = os.path.join(root_path, 'qkv_param_grads_avg.pt')
    mlp_gradient_save_path = os.path.join(root_path, 'mlp_param_grads_avg.pt')
    mlp_activation_save_path = os.path.join(root_path, 'mlp_param_activation.pt')
    assert (not Path(qkv_gradient_save_path).is_file() or
            not Path(mlp_gradient_save_path).is_file() or
            not Path(mlp_activation_save_path).is_file()), f"*** activation and grad files exist! ***"
    ...
    if 'llama' in model_args.model_name_or_path.lower():
        ...
        tokenizer.padding_side = "left"  # 对生成任务，左填充更安全
    logger.info(
    ...
    with training_args.main_process_first(desc="loading and tokenization"):
        ...
        logger.info(f"Training task names: {data_args.task_name}")
        file = os.path.join(path, f"{data_args.task_name}/train.json")
        analysis_dataset = build_instruction_dataset_from_json(
            data_file=file,
            tokenizer=tokenizer,
    ...
    config.use_cache = False  # suggest to be False, conflict with output_attentions
    ...
    logger.info(f"\n{'*' * 10}")
    logger.info(f"Starting analyzing on Task {data_args.task_name}: {len(analysis_dataset)} samples")
    logger.info(f"{'*' * 10}")

    # Initialize our Trainer
    trainer = QKVParameterGradientTrainer(
        model=model,
        args=training_args,
        qkv_gradient_save_path=qkv_gradient_save_path,
        mlp_gradient_save_path=mlp_gradient_save_path,
        mlp_activation_save_path=mlp_activation_save_path,
