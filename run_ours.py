...
class RegroupLoraMoECallback(transformers.TrainerCallback):
    ...
    def on_epoch_end(self, args, state, control, model=None, logs=None, **kwargs):
        ...
        # logs may be None, but the last entry in state.log_history usually has info
        last_log = {}
        if hasattr(state, 'log_history') and state.log_history:
            last_log = state.log_history[-1]  # dict like {'loss': ..., 'grad_norm': ..., 'learning_rate': ..., 'epoch': ...}

        # Build a readable log string
...
def main():
    ...
    # Dedicated logger: write to file only, do not output to console
    ...
    if not regroup_logger.handlers:
        ...
        # Key: do not add StreamHandler!

    # Add loss logger right after logging setup in main()
