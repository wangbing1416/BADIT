...
# Global constants (can be supplied externally)
...
def build_single_instruction_dataset_from_json(
    ...
    """
    Builds an instruction dataset from a single JSON file using the same format as `build_instruction_dataset`.

    Args:
        data_file: Path to the JSON file containing Definition and Instances.
        tokenizer: Hugging Face tokenizer.
        max_seq_length: Maximum sequence length.
        max_num_instances: Maximum number of samples to retain.
        data_cache_dir: Directory to cache processed data.
        preprocessing_num_workers: Number of workers for preprocessing.
        task_type: Task type label (e.g., "CL").

    Returns:
        Dataset: Torch dataset containing input_ids, labels, and task_types.
    """
    ...
    # Tokenization function (mirrors the original logic)
    def tokenization(examples):
        ...
        for instruction, input_text, output, task_type, subset in zip(
            ...
            # Merge instruction and input when provided
        ...
        # Tokenize sources (do not return the attention mask)
        tokenized_sources = tokenizer(sources, return_attention_mask=False)
        # tokenized_sources = sources
        # Tokenize targets (skip special tokens to avoid adding eos twice)
        ...
        for s, t in zip(tokenized_sources['input_ids'], tokenized_targets['input_ids']):
            ...
            labels = [IGNORE_INDEX] * len(s) + t  # Only compute loss on the output portion
    ...
    # Cache path: based on the filename and max_seq_length
    ...
    # Set the dataset to torch format
...
def left_pad_sequence(sequences, batch_first=False, padding_value=0):
    """
    Left-pad the sequences so padding appears before the data.
    """
